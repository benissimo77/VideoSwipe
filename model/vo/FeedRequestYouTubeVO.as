package com.videoswipe.model.vo 
{
	 ...
	 * @author 
	 */
	public class FeedRequestYouTubeVO 
	{
		...
		private var _format:String = "atom";	// format of response
		private var _searchText:String = "";	// search text
		private var _maxresults:int = 15;	// used to limit the returned number of results
		...
		{
			trace("FeedRequestYouTubeVO:: standardFeed:", f, _standardFeed);
			var r:RegExp = / +/g;
			f = f.replace(r, "+");
			if (f != _standardFeed) {
				_standardFeed = f;
		...
		public function get fullURL():String
		{
			...
			_u += "?v=2&format=5&max-results=" + _maxresults;
