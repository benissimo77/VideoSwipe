package com.videoswipe.view.component 
{
	 ...
	 * 
	 */
	public class Scroller extends XSprite
	{
		[Embed(source = "assets/scrollButton.png")]
		private var buttonClass:Class;
		[Embed(source = "assets/blue-scrollarrow-up.png")]
		...
		private var _target:ScrollableSprite;	// cache the target object
		...
		public function Scroller(_h:Boolean = false) 
		{
			_horizontal = _h;
			initView();
		}
		...
		public function set scrollTarget(_d:ScrollableSprite):void
		{
			if (_target) {
				removeChild(_target);
			}
			...
			if (_target) {
				...
				// for mobile we use the drag, for non-mobile we use the up/down graphics
				if (CONFIG::tablet) {
					...
					_target.addEventListener(TransformGestureEvent.GESTURE_SWIPE, swipeHandler);
				} else {
					addChild(_up);
					addChild(_down);
		...
		override public function redraw():void
		{
			...
			_targetMask.graphics.beginFill(0x000, 0);
			_targetMask.graphics.drawRect(0, 0, _width, _height);
			_targetMask.graphics.endFill();
			drawScrollButtons();
			adjustScrollButtonPositions();
		...
		private function drawScrollButtons():void
		{
			...
			if (_h < 48) _h = 48;	// minimum height of the scroll button
			...
			_upArrow.y = (_h - _upArrow.height) / 4;	// 4 nudges it closer to top
			_downArrow.x = (_w - _downArrow.width) / 2;
			_downArrow.y = (_h - _downArrow.height) / 4;
		...
		private function doScroll(e:TimerEvent = null):void
		{
			...
			if (_target.x != _x || _target.y != _y) {
				_target.scrolling = true;	// call setter can take action (eg cache bitmap version of itself)
				TweenLite.to( _target, 0.2, { x:_x, y:_y, ease:Linear.easeNone, onUpdate:adjustScrollButtonPositions, onComplete:doScroll } );
				trace("Scroller:: ", _y, _target.height);
			} else {
				_target.scrolling = false;	// clear scrolling flag on target (can eg restore interactivity)
			}
		}

		// implements a drag (on tablets allows target to slide up and down)
		private function mouseDownHandler(e:MouseEvent):void
		{
			trace("Scroller:: mouseDownHandler:" );
			_target.startDrag(false, new Rectangle(0, 0, 0, -_target.height));
		}
		private function mouseUpHandler(e:MouseEvent):void
		{
			trace("Scroller:: mouseUpHandler:" );
			_target.stopDrag();
		}
		...
		{
			trace("Scroller:: swipeHandler:", e.currentTarget.name, e.offsetX, e.offsetY );
			if (e.offsetY == 0) {
				// horizontal swipe do nothing
			} else {
				e.stopPropagation();	// prevents Stage from acting on it
				if (e.offsetY < 0) {
					_scrollSpeed = -120;
				} else {
					_scrollSpeed = 120;
		...
		private function rollOut(e:MouseEvent=null):void
		{
			...
			if (_scrolling) {
				// don't make invisible if scrolling, causes buttons to blink occasionally
			} else {
				_up.visible = false;
				_down.visible = false;
			}
